...
import {
  AddMembroProjetoDto,
  PapelProjeto,
} from './dto/add-membro-projeto.dto';
...
export class ProjetoService {
  ...
  async addMembro(
    idProjeto: number,
    idPerfilSolicitante: number,
    addMembroProjetoDto: AddMembroProjetoDto,
  ) {
    await this.podeEditarProjeto(idProjeto, idPerfilSolicitante);

    const idPerfilNovoMembro = (
      await this.prisma.perfil.findUnique({
    ...
    if (!novoMembro) {
      throw new NotFoundException(
        'Membro não encontrado na entidade do projeto',
      );
    }

    const classificacao =
      addMembroProjetoDto.papel === PapelProjeto.GERENTE
        ? 'GERENTE'
        : 'COLABORADOR';
  ...
  async removeMembro(
    ...
    await this.prisma.colaboradoresProjetos.deleteMany({
      where: { idProjeto, idMembro: membro.id },
    });

    return this.prisma.gerentesProjetos.deleteMany({
      where: { idProjeto, idMembro: membro.id },
    });
  }
  ...
  async updateMembro(
    idProjeto: number,
    idPerfilSolicitante: number,
    idPerfilAlterado: number,
    updateMembroProjetoDto: UpdateMembroProjetoDto,
  ) {
    await this.podeEditarProjeto(idProjeto, idPerfilSolicitante);
    ...
    if (updateMembroProjetoDto.papel === PapelProjeto.GERENTE) {
      await this.prisma.colaboradoresProjetos.deleteMany({
        where: {
          idProjeto,
          idMembro: membro.id,
  ...
  private async podeEditarProjeto(idProjeto: number, idPerfil: number) {
