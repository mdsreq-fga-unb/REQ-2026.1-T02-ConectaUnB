...
| ------ | ------ | ------ | ------ | ------ | ------ | ------ |
| Processos seletivos |  |  |  |  |  |  |
| RF-01 | Exibir processos seletivos abertos | O sistema deve permitir exibir todos os procesos seletivos abertos | CP5 | RF-1-01 | Must Have | O sistema deve exibir os processos seletivos abertos quando qualquer usuário (autenticado ou não) acessar a listagem de processos, garantindo que apenas as seleções com a data limite de inscrição não expirada sejam retornadas do banco de dados e exibidas na interface. |
| RF-02 | Filtrar processos seletivos abertos | O sistema deve permitir filtrar processos seletivos abertos | CP5 | RF-1-01 | Must Have | O sistema deve filtrar os processos seletivos abertos quando o usuário aplicar um ou mais parâmetros de categorização na busca, atualizando a listagem exibida na interface para corresponder exatamente às categorias selecionadas. |
| RF-03 | Atualizar status de um processo seletivo | O sistema deve permitir a atualização do status de um processo seletivo de uma entidade | CP4 | RF-1-06 | Must Have | O sistema deve atualizar o status de um processo seletivo quando o usuário com privilégios de gestor da entidade submeter a alteração no painel de gestão, refletindo a mudança imediatamente na interface pública. |
| RF-04 | Cancelar um processo seletivo | O sistema deve permitir o cancelamento de um processo seletivo de uma entidade | CP4 | RF-1-07 | Must Have | O sistema deve cancelar um processo seletivo quando o gestor da entidade confirmar a ação de cancelamento no painel, alterando seu estado para "Cancelado" e ocultando-o automaticamente da visualização do feed de processos abertos. |
| RF-05 | Criar um processo seletivo | O sistema deve permitir a criação de um processo seletivo de uma entidade delimitando data limite para inscrição | CP4 | RF-1-07 | Must Have | O sistema deve criar um processo seletivo quando o gestor da entidade submeter o formulário preenchido corretamente, validando obrigatoriamente a presença de uma data limite para inscrição válida antes de confirmar a criação do processo. |
| FEED |  |  |  |  |  |  |
| RF-06 | Filtrar feed de acordo com preferências do usuário | O sistema deve permitir a configuração de filtros de interesses pessoais (ex: equipes de competição, PIBIC) para personalizar o feed de visualização do aluno. | CP5 | RF-2-05 | Must Have | O sistema deve filtrar as publicações do feed quando o usuário autenticado tiver filtros de interesses pessoais configurados, restringindo a exibição exclusivamente às postagens que possuam as tags ou categorias correspondentes às suas preferências. |
| RF-07 | Divulgar publicações no formato de blog | O sistema deve exibir uma interface pública no formato de blog para a divulgação das atividades publicadas no perfil de uma entidade. | CP2 | RF-3-02 | Must Have | O sistema deve exibir as publicações de uma entidade quando o usuário acessar o perfil institucional correspondente, estruturadas em um formato de blog que apresente corretamente o título, o texto e os arquivos de mídia incluídos na postagem. |
| RF-08 | Interagir com publicações com botões de curtida | O sistema deve possuir um botão em cada publicação para que os usuários possam interagir através de curtidas | CP2 | RF-1-05 | Could Have | **Critério 1**: O sistema deve registrar a interação de curtida em uma publicação quando um usuário autenticado clicar no botão de curtir, incrementando o contador de interações da postagem e atualizando o número exibido na interface.**Critério 2**: O sistema deve remover a curtida de uma publicação quando o usuário autenticado clicar novamente no botão de curtir de um post já curtido por ele, decrementando o contador de interações e atualizando o número na interface. |
| PERFIL - ENTIDADE |  |  |  |  |  |  |
