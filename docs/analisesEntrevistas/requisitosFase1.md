...
| FEED | |  |  |  |
| RF-06 | Filtrar feed de acordo com preferências do usuário | O sistema deve permitir a configuração de filtros de interesses pessoais (ex: equipes de competição, PIBIC) para personalizar o feed de visualização do aluno. | *CPX* | RF-2-05 |
| RF-07 | Divulgar publicações no formato de blog | O sistema deve exibir uma interface pública no formato de blog para a divulgação das atividades de extensão cadastradas no perfil.| *CPX* | RF-3-02 |
| PERFIL | |  |  |  |
| RF-08 | Cadastrar perfil da entidade publicamente | O sistema deve permitir o cadastro de um perfil público de entidade contendo a descrição detalhada do propósito e das atividades. | *CPX* | RF-2-01 |
| RF-09 | Editar perfil da entidade | O sistema deve permitir a atualização dos dados de um perfil público de entidade. | *CPX* | RF-2-01 |
| RF-10 | Excluir perfil da entidade | O sistema deve permitir a exclusão dos dados de um perfil de entidade. | *CPX* | RF-2-01 |
| RF-11 | Acessar perfil da entidade | O sistema deve permitir a visualização dos dados de um perfil de entidade. | *CPX* | RF-2-01 |
| PROJETO | |  |  |  |
| RF-12 | Atualizar andamento de projetos | O sistema deve permitir a publicação de atualizações de projetos contendo texto e imagens. | *CPX* | RF-2-02 |
| RF-13 | Visualizar histórico de projetos | O sistema deve exibir o histórico de projetos anteriores e inativos no perfil de cada entidade. | *CPX* | RF-2-04 |
| RF-14 | Criar um novo projeto | O sistema deve permitir a publicação de atualizações de projetos contendo texto e imagens. | *CPX* | RF-2-02 |
| RF-15 | Deletar um projeto | O sistema deve permitir a deleção de projetos. | *CPX* | RF-2-02 |
| PUBLICAÇÕES | |  |  |  |
...
| RF-19 | Pesquisar publicações com filtros de busca | O sistema deve permitir a busca por posts através de filtros como tipo de entidade | CP4 CP6 | RF-3-03 |
| NOTIFICAÇÕES | |  |  |  |
| RF-20 | Notificar usuários personalizadamente | O sistema poderia permitir o usuário solicitar notificações de acordo com sua vontade | *CPX* | RF-1-03 RF-2-06|
| MÉTRICAS | |  |  |  |
| RF-21 | Exibir métricas de engajamento para as entidades | O sistema deve gerar um painel de métricas de acesso para a entidade exibindo o número de interações realizadas. | *CPX* | RF-2-07 |
| INTEGRAÇÃO | |  |  |  |
| RF-22 | Importar posts de outras redes | Seria legal se o sistema permitisse importar posts de outras redes sociais | CP2 CP4 CP6 | RF-1-04 |
